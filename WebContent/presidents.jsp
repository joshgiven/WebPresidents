	...
	<div class='chooser'>
		<form method='GET' action='lookup.do'>
			...
			<c:forEach var='president' items='${presidents}'>
				<option value='${president.ordinal}' <c:if test='${president.ordinal == trio.current.ordinal}'>selected</c:if>>${president.ordinal} ${president.lastName}</option>
			</c:forEach>
			</select>
			<button>Go</button>
		</form>
...
>>>>>>> e5d965d5db9e30d24e1ab51664a9c53b2de9abfe
	...
	<div class='prev'>
    <button name="previous" value="previous" formaction="lookup.do?view=pres&id=${trio.previous.ordinal}">Previous</button>
	...
	<div class='next'>
    <button name="next" value="next" formaction="lookup.do?view=pres&id=${trio.next.ordinal}">Next</button>
